...
20. Canonical key folds ALL separator punctuation to a space (supersedes the narrow fold in #14): general punctuation `. , ; : ! ? ( ) [ ] { } / \ | @ # $ % ^ & * + = ~`, underscore, every dash variant (`-`, U+2010–U+2015, U+2212) and every quote variant (straight `"`/`'`, U+2018–U+201F, `«»`) — quotes are separators now, not normalized-and-kept. Reason: dedup keys must converge for stylistic variants (““Acme”–Co Ltd” ≡ “Acme.Co Ltd” ≡ “Acme Co Ltd”); the previous fold left quoted vs unquoted and dot-separated names distinct. Storage is never touched; characters outside the enumerated set pass through. Fixed per updated domain spec 2026-07-16.
